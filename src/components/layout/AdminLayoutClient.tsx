...
export const AdminLayoutClient: React.FC<AdminLayoutClientProps> = ({
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-row font-cairo text-slate-800 overflow-x-hidden">
      ...
      {/* Main Content Area - Properly offset with lg:mr-72 in RTL so sidebar never overlaps */}
      <div className="flex-1 flex flex-col min-w-0 min-h-screen bg-[#F8FAFC] text-slate-800 lg:mr-72 transition-all">
        <SubscriptionBanner tenant={tenant} />
        <AdminHeader
          user={user}
          onToggleMobileSidebar={() => setMobileSidebarOpen(!mobileSidebarOpen)}
        />
        <main className="flex-1 p-4 sm:p-6 lg:p-8 pb-24 lg:pb-10 max-w-[1600px] w-full mx-auto">{children}</main>
