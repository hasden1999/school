...
export const PaymentReceiptModal: React.FC<PaymentReceiptModalProps> = ({
  ...
  const footerText = t?.printFooterText || "مدرسة المعالي الأهلية الابتدائية المختلطة (تأسست 2017) — وصل قبض واستلام مالي رسمي معتمد";
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="وصل قبض واستلام مالي رسمي" maxWidth="lg">
      <div className="space-y-6">
        ...
        <div className="bg-white p-6 sm:p-8 rounded-xl border border-slate-200 print-container shadow-md space-y-5 text-slate-800 font-cairo">
          ...
          <div className="flex items-center justify-between border-b-2 border-slate-300 pb-4">
            ...
            <div className="text-left font-mono">
              <span className="text-[10px] text-slate-500 font-sans block font-bold">رقم الوصل التسلسلي</span>
              <span className="font-bold text-slate-900 text-base bg-slate-100 px-3 py-1 rounded-lg border border-slate-200">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs bg-slate-50 p-4 rounded-lg border border-slate-200">
            <div>
              <span className="text-slate-500 text-[10px] block font-bold">وصلنا من الطالب / ولي الأمر:</span>
              <span className="font-bold text-slate-900 text-sm">{student.user?.fullName}</span>
            </div>
            <div>
              <span className="text-slate-500 text-[10px] block font-bold">الصف والشعبة:</span>
              <span className="font-bold text-slate-900">
                {student.classRoom?.name} ({student.section?.name})
              </span>
            </div>
            <div>
              <span className="text-slate-500 text-[10px] block font-bold">تاريخ القبض:</span>
              <span className="font-bold text-slate-900 font-mono">{receipt.paymentDate}</span>
            </div>
            <div>
              <span className="text-slate-500 text-[10px] block font-bold">طريقة الدفع:</span>
              <span className="font-bold text-slate-900">
          ...
          <div className="p-5 rounded-xl bg-brand-50 border border-brand-100 text-center space-y-1">
            <span className="text-xs text-brand-800 font-bold block">المبلغ المقبوض بموجب هذا الوصل:</span>
            <div className="text-3xl font-bold text-brand-900 font-mono tracking-wide">
              {Number(currentReceiptAmount).toLocaleString()} <span className="text-base font-sans font-bold">{currency}</span>
            </div>
            {receipt.notes && (
              <p className="text-xs text-brand-800 pt-1 font-semibold">
          ...
          <div className="rounded-lg border border-slate-200 overflow-hidden text-xs">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 divide-y sm:divide-y-0 sm:divide-x sm:divide-x-reverse divide-slate-200 bg-white">
              <div className="p-3 text-center">
                <span className="text-slate-500 text-[10px] block font-bold">القسط السنوي الكلي</span>
                <span className="font-bold text-slate-900 text-xs">
                  {Number(totalTuition).toLocaleString()} {currency}
                </span>
              </div>
              <div className="p-3 text-center">
                <span className="text-slate-500 text-[10px] block font-bold">المسدد سابقاً</span>
                <span className="font-semibold text-slate-700 text-xs">
                  {Number(previousPaid).toLocaleString()} {currency}
                </span>
              </div>
              <div className="p-3 text-center bg-brand-50/50">
                <span className="text-brand-700 text-[10px] block font-bold">المجموع المسدد حتى الآن</span>
          ...
          <div className="flex justify-between items-end pt-4 border-t border-slate-300 text-xs">
            <div className="space-y-1">
              <p className="text-slate-500 text-[10px] font-bold">المحاسب المسؤول</p>
