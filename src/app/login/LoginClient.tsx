      ...
      <header className="border-b border-slate-200 bg-white/95 backdrop-blur-xs sticky top-0 z-30 shadow-2xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            {/* Direct Register School Button in Header */}
            <button
              type="button"
              onClick={() => setShowRegisterModal(true)}
              className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-xs font-bold text-emerald-800 transition-colors shadow-2xs cursor-pointer"
            >
            ...
            <div className="lg:col-span-5 space-y-6 text-right order-2 lg:order-1">
              ...
              <div className="bg-gradient-to-br from-white to-emerald-50/40 border border-emerald-200/80 rounded-2xl p-4 sm:p-5 space-y-3 shadow-xs">
                ...
                <button
                  type="button"
                  onClick={() => setShowRegisterModal(true)}
                  className="w-full py-2.5 px-4 rounded-xl bg-emerald-800 hover:bg-emerald-900 text-white text-xs font-bold transition-all shadow-xs flex items-center justify-center gap-2 cursor-pointer"
                >
                  <PlusCircle className="w-4 h-4" />
                  <span>طلب تسجيل مدرسة جديدة أو تجربة المنظومة</span>
                </button>
              </div>
